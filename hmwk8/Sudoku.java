 ...
 *
 * Edited by:	Brandon Higashi
 * Mod:		October 29, 2013
 * Description:
 * 	Implemented fillSudoku using recursion and backtracking.
 */
...
public class Sudoku {
  ...
  public static boolean fillSudoku (int [] [] sudoku)
  {
    ...
    for(int i = 0; i < 9; i++) {
	    for(int j = 0; j < 9; j ++) {
		    // Continue if cell not equal to 0
		    if(sudoku[i][j] != 0) {continue;}
		    // Iterate from 1 to 9 to check if k is a valid number
		    for(int k = 1; k <= 9; k ++) {
			    // Set cell to k
			    sudoku[i][j] = k;
			    // Check if placing k is valid
			    // If valid then fillSudoku
			    if(checkSudoku(sudoku,false)) {
				    if(fillSudoku(sudoku)) {
					    return true;
				    }
			    }
			    // Backtrack if k is not valid
			    sudoku[i][j] = 0;
		    }
